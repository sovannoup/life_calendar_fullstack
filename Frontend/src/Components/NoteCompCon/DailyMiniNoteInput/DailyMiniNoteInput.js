        ...
        <DisableDisplayContainer>
            ...
            <IconListContainer>
                {iconList.map((item, index) => (
                    <i class={`bx bx-${item.icon} bx-style`} 
...
export default function DailyMiniNoteInput({setMessage, setIsNoted}) {
        ...
        <Container>
            ...
            <Header>
                ...
                <ZoomIconContainer disabled={isDisable} onClick={() => setOpen(true)}>
                    <svg  width="11" height="12" viewBox="0 0 11 12" fill={isDisable ? '#C0C0C0' : 'black'} xmlns="http://www.w3.org/2000/svg">
            ...
            <Body>
                {isDisable ?
                    ...
                    : (<TextArea placeholder="Is your day good?" value={note} onChange={event => setNote(event.target.value)} />)
                }

                <Footer>
                    <Button size='md' label="Save" disabled={isDisable} 
